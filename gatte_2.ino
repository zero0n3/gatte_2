...
//in base alla gatta apro o chiudo o non faccio nulla se già aperto
  ...
  case 0:    //nessuna gatta sulla bilancia
    if (sn > 0) {              //se una gatta è appena scesa
      ...
      if (myservo.read() > 90) {    //in particolare la tigre è scesa
        for (pos = myservo.read(); pos = 90; pos--) {    //faccio tornare indietro il servo dalla posizione in cui era
          myservo.write(pos);   
          delay(15);
        ...
        for (pos = myservo.read(); pos = 90; pos++) {  //faccio tornare indietro il servo
          myservo.write(pos);
          delay(15);
          SoftwareServo::refresh();
        }
  ...
  case 1:    //parte perla
    if (sn < 1 || sn > 1) {  //se era chiuso oppure la tigre è appena scesa
      ...
      if (myservo.read() < 135) {
        for (pos = myservo.read(); pos = 135; pos++) {
  ...
  case 2:    //parte tigre
    if (sn < 2) {
      ...
      if (myservo.read() > 45) {
        for (pos = myservo.read(); pos = 45; pos--) {
